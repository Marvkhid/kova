    ...
    <div className="min-h-screen bg-[#F5F0E8]">
      ...
      <div className="bg-[#0D0D0D] px-4 sm:px-5 md:px-8 py-3 flex items-center justify-between gap-3">
        ...
        <div className="flex items-center gap-2.5 sm:gap-3 flex-shrink-0">
          <Link href="/sellers/new" className="hidden sm:block text-[0.8rem] text-[#F5F0E8]/60 hover:text-[#F5F0E8] transition-colors">
            + Add product
          </Link>
          <div className="w-8 h-8 rounded-full bg-[#E8622A] flex items-center justify-center text-white text-sm font-bold">M</div>
        </div>
      </div>
      ...
      <div className="max-w-[1280px] mx-auto px-4 sm:px-5 md:px-8 py-6 sm:py-8">
        ...
        <div className="grid lg:grid-cols-[1fr_340px] gap-5 sm:gap-6 mb-6 sm:mb-8">
          ...
          <div className="bg-[#0D0D0D] rounded-[16px] sm:rounded-[20px] p-5 sm:p-6 flex flex-col justify-between">
            <div>
              <span className="text-[0.66rem] sm:text-[0.68rem] font-semibold text-[#E8622A] uppercase tracking-[0.1em]">💡 Tip of the week</span>
              <h3 className="font-bold text-[0.98rem] sm:text-[1.05rem] text-[#F5F0E8] mt-3 mb-2" style={{ fontFamily: 'var(--font-display)' }}>
            ...
            <Link href="/sellers/new" className="mt-5 sm:mt-6 inline-flex items-center gap-2 text-[0.8rem] sm:text-[0.82rem] font-medium text-[#E8622A] hover:opacity-70 transition-opacity">
              Update listings →
            </Link>
          </div>
        </div>
              ...
              {[
                ...
                <div
                  ...
                  className={`flex justify-between items-center py-3 ${
                    row.highlight ? 'bg-[#E8622A]/[0.06] px-4 rounded-[12px] -mx-4' : 'border-b border-black/[0.05]'
                  }`}
                >
                  <span className="text-[0.8rem] sm:text-[0.85rem] text-black/55">{row.label}</span>
                  <span className={`font-bold text-[0.9rem] sm:text-[0.95rem] ${row.highlight ? 'text-[#E8622A]' : 'text-[#0D0D0D]'}`}>
                    {row.value}
                  </span>
                </div>
              ))}
            </div>
            <button type="button" className="w-full mt-6 py-3 rounded-full bg-[#0D0D0D] text-[#F5F0E8] text-sm font-medium hover:bg-[#E8622A] transition-all duration-200">
              Request payout
            </button>
          </div>
        </div>
