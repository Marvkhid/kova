    ...
    <div className="bg-white rounded-[20px] border border-black/[0.07] overflow-hidden">
      ...
      <div className="flex items-center justify-between px-6 py-5 border-b border-black/[0.06]">
        <div>
          ...
          <p className="text-[0.75rem] text-black/40 mt-0.5">{PRODUCTS.length} products</p>
        </div>
        <Link
          href="/sellers/new"
          className="flex items-center gap-2 px-4 py-2 rounded-full bg-[#E8622A] text-white text-sm font-medium hover:bg-[#F07A48] hover:-translate-y-[1px] transition-all duration-200"
        >
          ...
          <thead>
            <tr className="border-b border-black/[0.05]">
              {['Product', 'Category', 'Price', 'Sales', 'Rating', 'Status', ''].map(h => (
                <th key={h} className="text-left px-6 py-3 text-[0.72rem] font-semibold text-black/35 uppercase tracking-[0.08em]">
                  {h}
                </th>
              ))}
            </tr>
                    ...
                    <div>
                      ...
                      {product.badge && (
                        <span className={`text-[0.65rem] font-semibold px-2 py-0.5 rounded-full ${
                          product.badge === 'new'  ? 'bg-black/[0.08] text-black/60' :
                          product.badge === 'hot'  ? 'bg-[#2A5C45]/10 text-[#2A5C45]' :
        ...
        {PRODUCTS.map(product => (
          <div key={product.id} className="flex items-center gap-3 px-5 py-4">
            ...
            <div className="flex-1 min-w-0">
              <p className="font-semibold text-[0.88rem] text-[#0D0D0D] truncate">{product.name}</p>
              <p className="text-[0.75rem] text-black/45">{formatPrice(product.price)} · {product.buyCount} sold</p>
            </div>
            <Link
              href={`/sellers/edit/${product.id}`}
              className="text-[0.75rem] text-[#E8622A] font-medium"
            >
              Edit
            </Link>
          </div>
