...
function HamburgerIcon({ open }: { open: boolean }) {
  return (
    <div className="w-5 h-4 relative flex flex-col justify-between" aria-hidden="true">
      {[0,1,2].map(i => (
        <span key={i} className="block h-[2px] bg-[#0D0D0D] rounded-full transition-all duration-300"
          style={{
...
function MobileDrawer({ open, onClose, pathname, itemCount, onCartClick, isSignedIn }:
  { open: boolean; onClose: () => void; pathname: string; itemCount: number; onCartClick: () => void; isSignedIn: boolean }) {
        ...
        <div className="px-5 pt-4 pb-6 flex flex-col gap-1">
          ...
          {NAV_LINKS.map(link => (
            <Link key={link.href} href={link.href} onClick={onClose}
              className={[
                'flex items-center justify-between px-4 py-3 rounded-[12px] text-base font-medium transition-all duration-200',
                pathname === link.href ? 'bg-[#0D0D0D] text-[#F5F0E8]' : 'text-[#0D0D0D] hover:bg-black/[0.06]',
              ].join(' ')}>
              {link.label}
              <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor"
                strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
          ...
          <div className="flex flex-col gap-2 pt-1">
            ...
            {isSignedIn && (
              <>
                ...
                <Link href="/sellers/dashboard" onClick={onClose}
                  className="text-center py-3 rounded-[12px] text-base font-medium text-[#0D0D0D] hover:bg-black/[0.06] transition-colors">
                  Dashboard
                </Link>
              </>
            ...
            <button onClick={() => { onClose(); onCartClick(); }}
