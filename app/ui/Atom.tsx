...
export function Badge({ type }: { type: ProductBadge }) {
  ...
  return (
    <span
      className={`inline-block text-[0.67rem] font-semibold tracking-[0.07em] uppercase px-[10px] py-1 rounded-full ${cfg.className}`}
    >
      {cfg.label}
    </span>
  );
...
export function StarRating({ rating, count }: { rating: number; count?: number }) {
  return (
    <div className="flex items-center gap-1.5">
      <span className="text-[#F4A438] text-sm leading-none">
        {'★'.repeat(Math.floor(rating))}
        {rating % 1 >= 0.5 ? '½' : ''}
        {'☆'.repeat(5 - Math.ceil(rating))}
      </span>
      {count !== undefined && (
        <span className="text-[0.72rem] text-black/40">({count})</span>
      )}
    </div>
  );
}
...
export function SectionLabel({ children, light = false }: { children: React.ReactNode; light?: boolean }) {
  return (
    <p
      className={`text-[0.7rem] font-medium tracking-[0.14em] uppercase mb-3 ${
...
export function EyebrowPill({ children }: { children: React.ReactNode }) {
  return (
    <span className="inline-block bg-[#E8622A]/[0.11] text-[#E8622A] text-[0.72rem] font-semibold tracking-[0.1em] uppercase px-[14px] py-[6px] rounded-full">
      {children}
    </span>
  );
}
...
export function Divider({ light = false }: { light?: boolean }) {
  return (
    <hr className={`border-0 border-t ${light ? 'border-white/10' : 'border-black/8'}`} />
  );
}