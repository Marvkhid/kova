      ...
      >
        {preview ? (
          ...
          <div className="w-full h-full bg-[#E8622A] flex items-center justify-center">
            <span className="font-extrabold text-white text-[1.8rem]"
        ...
        <div className="absolute inset-0 bg-black/40 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center">
          <span className="text-white text-[0.7rem] font-medium">Change</span>
        </div>
      </div>
...
export default function ProfilePage() {
  ...
  return (
    <div className="min-h-screen bg-[#F5F0E8] pb-24 md:pb-0">

      {/* Header */}
      <div className="bg-[#0D0D0D] pt-10 pb-16">
      ...
      <div className="max-w-[1280px] mx-auto px-5 md:px-8 py-10">
        <div className="grid lg:grid-cols-[300px_1fr] gap-8 items-start">
          ...
          <div className="flex flex-col gap-4">
            ...
            {/* Profile card */}
            <div className="bg-white rounded-[20px] border border-black/[0.07] p-6 flex flex-col items-center text-center gap-2">
              <AvatarUploader name={name} />
              <p className="font-bold text-[1rem] text-[#0D0D0D] mt-2"
                style={{ fontFamily: 'var(--font-display)' }}>
                {name}
              </p>
              <p className="text-[0.78rem] text-black/40">{email}</p>
              <p className="text-[0.82rem] text-black/50 mt-1">{bio}</p>
            </div>
            ...
            <div className="bg-white rounded-[20px] border border-black/[0.07] overflow-hidden">
              {[
                ...
                <Link key={item.href} href={item.href}
                  className={`flex items-center gap-3 px-5 py-3.5 hover:bg-black/[0.03] transition-colors text-[0.88rem] font-medium text-[#0D0D0D] ${
          ...
          <div className="flex flex-col gap-6">
            ...
            <div className="bg-white rounded-[20px] border border-black/[0.07] p-6">
              <h2 className="font-bold text-[1rem] text-[#0D0D0D] mb-5"
                style={{ fontFamily: 'var(--font-display)' }}>
              ...
              <form onSubmit={handleSave} className="flex flex-col gap-4">
                <div className="grid sm:grid-cols-2 gap-4">
                  <div className="flex flex-col gap-1.5">
                    <label className="text-[0.8rem] font-semibold text-black/60">Full name</label>
                    <input type="text" value={name} onChange={e => setName(e.target.value)}
                      className="h-[44px] px-4 rounded-[10px] bg-[#F5F0E8] border border-black/[0.09] text-[0.88rem] text-[#0D0D0D] outline-none focus:border-[#E8622A] focus:ring-2 focus:ring-[#E8622A]/15 transition-all"/>
                  </div>
                  <div className="flex flex-col gap-1.5">
                    <label className="text-[0.8rem] font-semibold text-black/60">Email address</label>
                    <input type="email" value={email} onChange={e => setEmail(e.target.value)}
                      className="h-[44px] px-4 rounded-[10px] bg-[#F5F0E8] border border-black/[0.09] text-[0.88rem] text-[#0D0D0D] outline-none focus:border-[#E8622A] focus:ring-2 focus:ring-[#E8622A]/15 transition-all"/>
                  </div>
                </div>
                <div className="flex flex-col gap-1.5">
                  <label className="text-[0.8rem] font-semibold text-black/60">Bio</label>
                  <textarea value={bio} onChange={e => setBio(e.target.value)} rows={3}
                    className="px-4 py-3 rounded-[10px] bg-[#F5F0E8] border border-black/[0.09] text-[0.88rem] text-[#0D0D0D] outline-none focus:border-[#E8622A] focus:ring-2 focus:ring-[#E8622A]/15 transition-all resize-none"/>
                </div>
            ...
            <div className="bg-white rounded-[20px] border border-black/[0.07] overflow-hidden">
              <div className="flex items-center justify-between px-6 py-4 border-b border-black/[0.06]">
                <h2 className="font-bold text-[1rem] text-[#0D0D0D]"
                  style={{ fontFamily: 'var(--font-display)' }}>
                  Recent orders
                </h2>
                <Link href="/orders"
              ...
              <div className="divide-y divide-black/[0.05]">
                {RECENT_ORDERS.map(order => (
                  <div key={order.id} className="flex items-center gap-4 px-6 py-4 hover:bg-black/[0.018] transition-colors">
                    <div className="w-11 h-11 rounded-[10px] overflow-hidden bg-[#EDE8DF] flex-shrink-0">
                      <img src={`/images/${order.product.imagePlaceholder}.jpg`} alt={order.product.name}
                        style={{ width: '100%', height: '100%', objectFit: 'cover' }}/>
                    </div>
                    <div className="flex-1 min-w-0">
                      <p className="font-medium text-[0.85rem] text-[#0D0D0D] truncate">{order.product.name}</p>
                      <p className="text-[0.72rem] text-black/38 mt-0.5">{order.id} · {order.date}</p>
                    </div>
                    <span className={`text-[0.68rem] font-semibold px-2.5 py-1 rounded-full capitalize flex-shrink-0 ${STATUS_STYLES[order.status]}`}>
                      {order.status}
                    </span>
                    <p className="font-bold text-[0.88rem] text-[#0D0D0D] flex-shrink-0"
                      style={{ fontFamily: 'var(--font-display)' }}>
