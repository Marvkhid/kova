...
function TabSwitcher({ active, onChange }: { active: Tab; onChange: (t: Tab) => void }) {
  return (
    <div className="flex bg-black/[0.06] rounded-full p-1 mb-7 sm:mb-8">
      {(['login', 'signup'] as Tab[]).map((tab) => (
        <button
          ...
          className={[
            'flex-1 py-2 rounded-full text-[0.88rem] sm:text-sm font-medium transition-all duration-200 capitalize',
            active === tab ? 'bg-white text-[#0D0D0D] shadow-sm' : 'text-black/50 hover:text-black/80',
          ].join(' ')}
        >
...
export default function LoginPage() {
  ...
  return (
    <div className="min-h-screen flex flex-col lg:flex-row">
      ...
      <div className="relative w-full h-[220px] xs:h-[260px] sm:h-[320px] lg:hidden overflow-hidden">
        <img
        ...
        <div className="absolute bottom-4 sm:bottom-6 left-4 sm:left-6 right-4 sm:right-6 flex gap-4 sm:gap-6 z-10">
          {[
            { n: '48K+', l: 'Sellers' },
            { n: '190+', l: 'Countries' },
            { n: '4.9★', l: 'Rating' },
            ...
            <div key={s.n}>
              <p
                className="font-extrabold text-[1rem] sm:text-[1.2rem] text-[#F5F0E8] leading-none"
                style={{ fontFamily: 'var(--font-display)' }}
              >
                {s.n}
              </p>
              <p className="text-[0.66rem] sm:text-[0.72rem] text-[#F5F0E8]/55 mt-0.5">{s.l}</p>
            </div>
          ))}
        </div>
      </div>
        ...
        <div className="relative z-10 mt-auto">
          ...
          <div className="flex gap-6 mt-8">
            {[
              ...
              <div key={s.n}>
                <p
                  className="font-extrabold text-[1.4rem] text-[#F5F0E8] leading-none"
                  style={{ fontFamily: 'var(--font-display)' }}
                >
                  {s.n}
                </p>
      ...
      {/* ── Form panel ── */}
      <div className="flex-1 flex flex-col justify-center bg-[#F5F0E8] px-4 sm:px-6 py-8 sm:py-10 lg:px-14 xl:px-20">
        <div className="w-full max-w-[420px] mx-auto">
          {/* Logo */}
          <Link href="/" className="inline-flex items-center gap-2 mb-7 sm:mb-8">
            ...
            <span
              className="font-extrabold text-[1.15rem] sm:text-[1.25rem] tracking-[-0.03em] text-[#0D0D0D]"
              style={{ fontFamily: 'var(--font-display)' }}
            >
          ...
          <div className="mb-6 sm:mb-7">
            <h1
              ...
              style={{ fontFamily: 'var(--font-display)', fontSize: 'clamp(1.5rem, 6vw, 2rem)' }}
            >
              {tab === 'login' ? 'Welcome back.' : 'Join KOVA.'}
            </h1>
            <p className="text-[0.86rem] sm:text-[0.9rem] text-black/48">
              {tab === 'login'
